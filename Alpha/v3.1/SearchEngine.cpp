...
{
  //no data members, nothing to init
}

SearchEngine::~SearchEngine()
{
  //no data members, nothing to clean up
}

/*
  Take in a lattice and run the SearchEngine algorithm over it.
...
*/
void SearchEngine::Process(Lattice& lattice, LatticePaths& wordList)
{
  //double maxArc;
  ...
  SimpleViterbi(lattice,wordList);

  //run SearchEngine algorithm. Currently only returns the single most-likely word, instead of some permutation of the input code.
...
void SearchEngine::DFS(State* curState, char prefix[256], double cumulativeProb, int curDepth, const int depthBound, LatticePaths& resultList)
{
...
*/
double SearchEngine::WorstBestHeuristic(Lattice& lattice)
{
  int i, j, bestState, worstState;
...
/*
  ...
  Likely the threshold would be determined by some probe: could perhaps use SearchEngine for this, since it finds the best path
  in linear time; then apply some scalar to the log probability of the best path. For example, if SearchEngine gives back likeliest
...
*/
void SearchEngine::PrunedDFS(State* curState, char prefix[256], double cumulativeProb, const double pruneThresholdProb, int curDepth, const int depthBound, LatticePaths& resultList)
{
  double prob;
  prefix[curDepth-1] = curState->symbol;
...
*/
double SearchEngine::ThresholdHeuristic(LatticePaths& subList)
{
  LatticePathsIt it;
  long double max, min;
  //dbg only
...
*/
void SearchEngine::RunPrunedSearch(Lattice& lattice, LatticePaths& results, int depthBound)
{
  int depth;
  char prefix[256];
...
/*
  Viterbi only returns the most likely path through the graph, whereas we may be interested in getting a list of all
...
*/
void SearchEngine::RunExhaustiveSearch(Lattice& lattice, LatticePaths& results, int depthBound)
{
  int depth;
  char prefix[256];
...
void SearchEngine::PrintResultList(LatticePaths& results)
{
  U32 i;
  LatticePathsIt it;
...
/*
  First-order graphs are those for which edge-weights don't matter, only state probabilities do.
  This makes searching for the most likely path a simple matter of selecting the maximum-probablity
  state as you move from left to right through the lattice. At each stage, just select the max
  state.

  Returns: Adds only a single result to the result-list, the most likely path (given no conditioning data).
*/
void SearchEngine::SimpleViterbi(Lattice& lattice, LatticePaths& results)
{
  int i;
  LatticePath message;
  message.second = 0.0; 

  cout << "Running SimpleViterbi, assuming max state is the last row-entry in each column..." << endl;
  for(i = 0; i < lattice.size(); i++){
    message.first += lattice[i].alphas[lattice[i].alphas.size()-1].symbol; //take vals of last state directly (!)
    message.second += lattice[i].alphas[lattice[i].alphas.size()-1].pState;
    //cout << "i=" << i << endl;
  }
  //cout << "done..." << endl;
...
*/
void SearchEngine::RunViterbi(Lattice& lattice, LatticePaths& results)
{
  int i, j, k;
  double mostLikely;
  State* endState;
...
//cout << "last col n_alphas: " << lattice[lattice.size()-1].alphas.size() << endl;
  ...
  //iterate forward (i think textbooks call this forward-messaging, but I'm not sure its equivalent)
  for(i = 0; i < lattice.size() - 1; i++){
  ...
  if(endState == NULL){
    cout << "ERROR endState null in RunSearchEngine()" << endl;
  }
...
*/
void SearchEngine::SortArcs(Lattice& lattice)
{
  int i, j;
...
*/
void SearchEngine::AbsorbStateProbabilitiesInArcs(Lattice& lattice)
{
  int i, j, k;
...
char SearchEngine::GetNextMaxAlphaCluster(const Cluster& cluster, )
{
...
char SearchEngine::GetMaxAlphaFromCluster(const Cluster& cluster)
{
  char c;
  double max = 0.0;

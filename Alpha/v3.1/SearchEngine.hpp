...
/*
  ...
  I named this SearchEngine after the core algorithm, but this is likely to change, since SearchEngine only returns the most likely output.
  In reality this class will gradually encompass a bunch of graph search behavior, not just SearchEngine.


  
  Requirements: 
  Conditioner sends arcs fully conditioned, but does NOT absorb state probabilities into arcs. SearchEngine will deploy these strategies,
  however they may be needed (absorbing, sorting, etc).
  
  For now, the strategy for SearchEngine will be to accept a Lattice in the above state, absorb state probabilities into arc probabilities,
  ...
  TODO: If I stick with first-order lattice (no edge weights), could rewrite things much more simply. SearchEngine wouldn't even
  ...
  Output of SearchEngine is a set of string-probability pairs defining the most-tightly defined set of possible strings given by the most likely

...
public class DirectInference
{
  ...
  public DirectInference(String vocabFileName, KeyMap kmap){
    ...
    _keyMapRef = kmap;
    _vocab = new Vocab(vocabFileName);
  }
  ...
  */
  private double SumDistMetric_Aligned(ArrayList<PointMu> pointMeans, String candidate)
  {
      ...
      if(pointMeans.get(i).GetAlpha() != candidate.charAt(j)){
        ...
        if((i+1) < pointMeans.size() && pointMeans.get(i+1).GetAlpha() == candidate.charAt(j)){
          if(j > 0){  //accumulate error, so this character difference receives at least some punishment
            sumDist += InsertionError(pointMeans.get(i).GetPoint(), _keyMapRef.GetPoint(candidate.charAt(j-1)), _keyMapRef.GetPoint(candidate.charAt(j)));
          }
          else{ //the first is just an exception case, when we don't have two points to compare. so just compare char i to j
            sumDist += Point.DoubleDistance(pointMeans.get(i).GetPoint(), _keyMapRef.GetPoint(candidate.charAt(j)));
          }
        ...
        //In this case, do the opposite of the insertion error case, advancing candidate index
          ...
          if(i > 0 && pointMeans.size() > 1){
            sumDist += Point.DoubleDistance(pointMeans.get(i-1).GetPoint(), _keyMapRef.GetPoint(candidate.charAt(j)));
            //sumDist += InsertionError(_keyMapRef.GetPoint(candidate.charAt(j)), pointMeans.get(i).GetPoint, pointMeans.get(i-1).GetPoint());
          }
    ...
    while(i < pointMeans.size()){
      sumDist +=  Point.DoubleDistance(pointMeans.get(i).GetPoint(), _keyMapRef.GetPoint(candidate.charAt(j-1)));
      i++;
    }
    ...
    sumDist += ((double)rpts * _keyMapRef.GetMinInterKeyRadius() * 0.15); //token punishment to separate FOXX from FOX, DOGG from DOG

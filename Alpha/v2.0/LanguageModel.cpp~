...
void LanguageModel::ReconditionByCharGrams(LatticePaths& edits)
{
  ...
  for(LatticePathsIt it = edits.begin(); it != edits.end(); ++it){
    ...
    for(i = 0; i < it->first.size() - 1; i++){
      ...
      ngram_val += (CHAR_BIGRAM_LAMBDA * GetBigramProbability(it->first[i],it->first[i+1]));
    }
    //trigram conditioning
    for(i = 0; i < it->first.size() - 2; i++){
      ngram_val += (CHAR_TRIGRAM_LAMBDA * GetTrigramProbability(it->first[i],it->first[i+1],it->first[i+2]));
    }
    //quadgram conditioning
    for(i = 0; i < it->first.size() - 3; i++){
      ngram_val += (CHAR_QUADGRAM_LAMBDA * GetQuadgramProbability(it->first[i],it->first[i+1],it->first[i+2],it->first[i+3]));
    }
    //pentagram conditioning
    for(i = 0; i < it->first.size() - 4; i++){
      ngram_val += (CHAR_PENTAGRAM_LAMBDA * GetPentagramProbability(it->first[i],it->first[i+1],it->first[i+2],it->first[i+3],it->first[i+4]));
    }
...
void LanguageModel::MajorityVoteFilter(LatticePaths& paths, int topN, int k, vector<string>& output)
{
  ...
  for(col = 0; col < latticeWidth; col++){
    ...
    for(j = 0, inner = paths.begin(); inner != paths.end() && j < topN; ++inner, j++){
      ++charMap[ inner->first[col] ];
      cout << "val=" << (U32)inner->first[col] << endl;
      if(charMap[ inner->first[col] ] > max){
        max = charMap[ inner->first[col] ];
...
*/
void LanguageModel::ShallowSearch(LatticePaths& edits, int depth)
{
  int i;
  LatticePathsIt it;
  LatticePaths output;
  ...
  while(infile.getline(buf,BUFSIZE)){  // same as: while (getline( myfile, line ).good())
    ...
    if(ntoks == 2){
      ...
      switch(n){
        ...
        case 5: 
            key = GetPentaKey(tokens[0][4], tokens[0][3], tokens[0][2], tokens[0][1], tokens[0][0]);
            //cout << "entering tokens[0] >" << tokens[0] << "< with log-probability=" << tokens[1] << "  (verify correctness)" << endl;
...
{
  CharGramIt it = unigramModel.find(a);
...
{
  CharGramIt it = bigramModel.find(((U32)b << 8) | (U32)a);
...
{
  CharGramIt it = trigramModel.find(((U32)c << 16) | ((U32)b << 8) | (U32)a );
...
{
  CharGramIt it = quadgramModel.find( ((U32)d << 24) | ((U32)c << 16) | ((U32)b << 8) | (U32)a );  //returns zero if these elements are new to the model, which they never should be
...
//which requires a compression scheme of converting a char to 6-bit encoding. This is handled by CharToCompChar().
double LanguageModel::GetPentagramProbability(char a, char b, char c, char d, char e)
{
  CharGramIt it = pentagramModel.find( GetPentaKey(a,b,c,d,e) );
...
U32 LanguageModel::GetPentaKey(char a, char b, char c, char d, char e)
{
  return (CharToCompChar(e) << 26) | (CharToCompChar(d) << 20) | (CharToCompChar(c) << 14) | (CharToCompChar(b) << 8) | CharToCompChar(a);
}
...
*/
U32 LanguageModel::CharToCompChar(char c)
{
  return (U32)c - (U32)'A';
}
